    ...
    def handle_client(self, client_socket):
        ...
        while True:
            ...
            if msg_client.startswith("PEDIR"):
                self.realizar_pedido(self, client_socket, celular_registrado, msg_client, Restaurante)
    ...
    def cadastrar_item_cardapio(self, client_socket, msg_client):
        with self.__lock_pedidos:
            ...
            if cardapio.get(nome) is not None:
                ...
                prato = Prato(nome, descricao, preco)
                cardapio.put(prato.nome, prato.descricao)
                resposta = "202"  # Código para item cadastrado com sucesso

            client_socket.send(resposta.encode())

    def realizar_pedido(self, client_socket, celular_registrado, msg_client, Restaurante):
        with self.__lock_pedidos:
            num = celular_registrado
            _, nome_produto = msg_client.split()

            produto = Restaurante.cardapio.get(nome_produto)
